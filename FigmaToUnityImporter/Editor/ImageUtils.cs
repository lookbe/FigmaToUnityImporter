...
namespace FigmaImporter.Editor
{
    public class ImageUtils
    {
        ...
        public static async Task RenderNodeAndApply(Node node, GameObject nodeGo, FigmaImporter importer)
        {
            FigmaNodesProgressInfo.CurrentInfo = "Loading image";
            FigmaNodesProgressInfo.ShowProgress(0f);
            string nodeID = importer.CheckDuplicateNode(FigmaImporter.GetNodeId(node.id));
            var result = await importer.GetImage(nodeID.Replace("__", ";").Replace("_", ":"), true);
            var t = nodeGo.transform as RectTransform;
            //string spriteName = $"{node.name}_{nodeID.Replace(':', '_').Replace(";", "__")}.png";
            string spriteName = $"{nodeID}.png";
            ...
            if (result != null)
            {
                bool exist = SaveTexture(result, $"/{importer.GetRendersFolderPath()}/{spriteName}");
                
                if (exist)
                    sprite = AssetDatabase.LoadAssetAtPath<Sprite>($"Assets/{importer.GetRendersFolderPath()}/{spriteName}");
                else
